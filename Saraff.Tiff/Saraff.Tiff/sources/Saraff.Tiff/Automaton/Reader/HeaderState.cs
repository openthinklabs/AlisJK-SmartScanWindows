...
namespace Saraff.Tiff.Automaton.Reader {
    ...
    internal sealed class HeaderState:ReaderStateBase {
        internal const string ToIfd="To_First_IFD";

        #region IReaderAutomaton

        public override void ReadHeader() {
            var _header=Helper.Read<TiffHeader>(this.Context.Reader.BaseStream);
            if(_header.magic!=MagicValues.LittleEndian) {
                throw new NotSupportedException("Not supported format.");
            }
            this.Context.Reader.TiffVersion=_header.version;
            this.Context.NextImageFileDirectory=_header.dirOffset;

            this.FireEvent(HeaderState.ToIfd);
        }
